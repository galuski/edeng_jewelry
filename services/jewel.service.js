...
async function query(filterBy = {}) {
    ...
    // ב-MongoDB, אם השדה במסד הוא מערך (למשל ["Rings", "Gold"]),
    // שאילתה פשוטה כמו { type: "Rings" } תמצא את המסמך הזה.
    // לכן אין צורך בשינוי מיוחד כאן, זה יעבוד אוטומטית.
...
// --------------------------------------------------
// 💾 שמירת תכשיט (הוספה או עדכון)
// --------------------------------------------------
async function save(jewel) {
    const collection = await dbService.getCollection(COLLECTION_NAME)

    // ✅ נרמול שדה Type:
    // אנחנו מוודאים ש-type תמיד נשמר כמערך במסד הנתונים.
    // אם הוא מגיע כמחרוזת (מוצר ישן/באג), נהפוך למערך.
    // אם הוא לא קיים, נשים מערך ריק.
    let typeToSave = [];
    if (jewel.type) {
        typeToSave = Array.isArray(jewel.type) ? jewel.type : [jewel.type];
    }
    ...
    if (jewel._id) {
        ...
        const jewelToUpdate = {
            ...
            type: typeToSave, // שמירה המערך המנורמל
        ...
        // 🆕 הוספה חדשה
        // מעדכנים את האובייקט לפני השמירה
        jewel.type = typeToSave;
