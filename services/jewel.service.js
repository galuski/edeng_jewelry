...
async function query(filterBy = {}) {
    ...
    if (filterBy.txt) {
        criteria.vendor = { $regex: filterBy.txt, $options: 'i' }; // חיפוש חלקי בשם המוכר
    }
...
async function remove(jewelId, loggedinUser) {
    ...
    if (jewel.owner._id.toString() !== loggedinUser._id.toString()) throw new Error('Not your jewel');
...
async function save(jewel, loggedinUser) {
    ...
    if (jewel._id) {
        ...
        await collection.updateOne({ _id: new ObjectId(jewel._id) }, { $set: jewelToUpdate });
        return jewel;
    } else {
        // **הוספה**
